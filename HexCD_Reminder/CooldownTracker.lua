...
local CT = {}
HexCDReminder.CooldownTracker = CT
...
--- Usage in macro: /run HexCDReminder.CooldownTracker:OnManualCast(391528)
---@param spellID number
...
function CT:GenerateMacro(spellID)
    ...
    local macro = string.format(
        "#showtooltip %s\n/cast %s\n/run HexCDReminder.CooldownTracker:OnManualCast(%d)",
...
function CT:PrintMacros()
    ...
    for _, entry in ipairs(sorted) do
        ...
        print(string.format("    /run HexCDReminder.CooldownTracker:OnManualCast(%d)", entry.id))
    end
end
