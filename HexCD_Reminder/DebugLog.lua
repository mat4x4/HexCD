...
local function getLogLevel()
    local cfg = HexCDReminderDB and HexCDReminderDB.logLevel or "INFO"
...
function Log:OnFightEnd(success)
    ...
    -- Auto-save fight log
    if currentFightInfo and HexCDReminderDB then
        HexCDReminderDB.fightLogs = HexCDReminderDB.fightLogs or {}
        ...
        if HexCDReminder.GetAbilityTimeline then
            local timeline = HexCDReminder.GetAbilityTimeline()
        ...
        if HexCDReminder.GetSpellFireCounts then
            local counts = HexCDReminder.GetSpellFireCounts()
            if counts then
                savedFireCounts = {}
        ...
        table.insert(HexCDReminderDB.fightLogs, fightLog)

        -- Keep only last N fights
        while #HexCDReminderDB.fightLogs > MAX_SAVED_FIGHTS do
            table.remove(HexCDReminderDB.fightLogs, 1)
        end

        self:Log("INFO", string.format("Fight log auto-saved (%d entries, %d total saved fights)",
            #savedEntries, #HexCDReminderDB.fightLogs))
    end
...
function Log:OnSessionEnd(completed)
    ...
    self:Log("INFO", string.format("=== M+ SESSION END: %s (%s, %.0fs) ===",
        result, HexCDReminder.Util.FormatTime(duration), duration))

    -- Save full session log to SavedVariables
    if HexCDReminderDB then
        HexCDReminderDB.sessionLogs = HexCDReminderDB.sessionLogs or {}
        ...
        table.insert(HexCDReminderDB.sessionLogs, sessionLog)

        -- Keep only last 10 session logs (keys are long, entries are big)
        while #HexCDReminderDB.sessionLogs > 10 do
            table.remove(HexCDReminderDB.sessionLogs, 1)
        end

        self:Log("INFO", string.format("M+ session log saved (%d entries, %d total sessions)",
            #savedEntries, #HexCDReminderDB.sessionLogs))
    end
...
function Log:ExportSession()
    if not sessionInfo and (not HexCDReminderDB or not HexCDReminderDB.sessionLogs or #HexCDReminderDB.sessionLogs == 0) then
    ...
    if sessionInfo then
        ...
        table.insert(lines, string.format("Duration: %s (IN PROGRESS)", HexCDReminder.Util.FormatTime(duration)))
        table.insert(lines, "")
        ...
        for _, e in ipairs(allEntries) do
            ...
            local ok, line = pcall(function()
                local timeStr = e.sessionRelativeSec
                    and string.format("[%s]", HexCDReminder.Util.FormatTime(e.sessionRelativeSec))
                    or "[--:--]"
    ...
    local log = HexCDReminderDB.sessionLogs[#HexCDReminderDB.sessionLogs]
    local lines = {}
    table.insert(lines, string.format("=== HexCD M+ Session Log ==="))
    table.insert(lines, string.format("Dungeon: %s +%d (mapID=%d)",
    ...
    table.insert(lines, string.format("Duration: %s | Result: %s",
        HexCDReminder.Util.FormatTime(log.duration), log.completed and "COMPLETED" or "ABANDONED"))
...
function Log:GetSavedSessions()
    if not HexCDReminderDB or not HexCDReminderDB.sessionLogs then return {} end
    local result = {}
    for i, log in ipairs(HexCDReminderDB.sessionLogs) do
        table.insert(result, {
            index = i,
...
function Log:ExportSavedSession(index)
    if not HexCDReminderDB or not HexCDReminderDB.sessionLogs then return "No saved sessions." end
    local log = HexCDReminderDB.sessionLogs[index]
...
function Log:Export()
    ...
    for _, e in ipairs(allEntries) do
        local timeStr = e.fightRelativeSec
            and string.format("[%s]", HexCDReminder.Util.FormatTime(e.fightRelativeSec))
            or "[--:--]"
